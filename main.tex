...
Figure~\ref{fig:distributed} and Table~\ref{tab:distributed} show distributed CG on 4 NVIDIA H200 GPUs with NCCL backend.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/distributed_benchmark.png}
\caption{Multi-GPU scaling performance: distributed CG with NCCL backend on 4$\times$ H200 GPUs, showing time and memory scaling across problem sizes.}
\label{fig:distributed}
\end{figure}

\begin{table}[t]
\centering
\caption{Distributed solve on 4$\times$ H200 GPUs (NCCL) vs. 4 CPU processes (Gloo).}
\label{tab:distributed}
\vspace{0.5em}
\small
\begin{tabular}{rrrrr}
\toprule
\textbf{DOF} & \textbf{GPU Time} & \textbf{CPU Time} & \textbf{Memory/GPU} & \textbf{Speedup} \\
\midrule
10K & 0.18 s & 0.37 s & 0.03 GB & 2.1$\times$ \\
100K & 0.61 s & 7.42 s & 0.05 GB & 12.2$\times$ \\
500K & 1.64 s & -- & 0.15 GB & -- \\
1M & 2.82 s & -- & 0.27 GB & -- \\
2M & 6.02 s & -- & 0.50 GB & -- \\
\bottomrule
...
\textbf{Scaling analysis.} The $12\times$ GPU speedup at 100K DOF arises from two factors:

\textit{(1) Compute advantage:} H200 delivers 3.9 TFLOPS (FP64) vs. $\sim$100 GFLOPS for CPU cores, a $40\times$ peak ratio. SpMV achieves $\sim$15\% of peak on both platforms due to memory-bound nature.

\textit{(2) Communication advantage:} NCCL peer-to-peer achieves 400 GB/s (NVLink) vs. Gloo's $\sim$10 GB/s (network), reducing halo exchange latency by $40\times$.

At small problems (10K DOF), the speedup drops to $2\times$ because communication overhead dominates: each CG iteration requires 2 \texttt{all\_reduce} operations with $\cO(\log P)$ latency, amortized only when local computation is substantial.

\textbf{Memory distribution.} Each GPU stores $n/P$ owned nodes plus $|\mathcal{H}_p|$ halo nodes. For structured grids, $|\mathcal{H}_p| \sim \cO(\sqrt{n/P})$, so halo overhead diminishes with problem size. At 2M DOF, halo overhead is $<$1\%.

\textbf{Theoretical limits.} With 4 $\times$ 140GB and 443 bytes/DOF, maximum capacity is $\sim$1.3B DOF. Communication becomes the bottleneck: at this scale, halo exchange transfers $\sim$100MB per iteration, taking $\sim$0.25ms on NVLink---comparable to SpMV compute time. This suggests $\sim$50\% parallel efficiency at billion-scale DOF.
