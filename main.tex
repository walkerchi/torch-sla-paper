...
\label{alg:halo}
\begin{algorithmic}[1]
\Require Local vector $\bx_{\text{local}}$, neighbor information
\State \textbf{Send} owned boundary values to neighbors (async)
\State \textbf{Receive} halo values from neighbors (async)
\State \textbf{Synchronize} communication
\State $\by_{\text{owned}} \gets \bA_{\text{local}} \bx_{\text{local}}$ \Comment{Includes halo columns}
\State \Return $\by_{\text{owned}}$
\end{algorithmic}
\end{algorithm}
...
\label{alg:dist_cg}
\begin{algorithmic}[1]
\Require Distributed matrix $\bA$, local RHS $\bb_{\text{owned}}$, tolerance $\epsilon$
\State $\bx \gets \mathbf{0}$, $\br \gets \bb_{\text{owned}}$, $\bp \gets \br$
\State $\rho \gets \text{all\_reduce}(\br^\top \br, \text{SUM})$ \Comment{Global dot product}
\While{$\sqrt{\rho} > \epsilon$}
    \State $\bA\bp \gets \textsc{DistributedSpMV}(\bA, \bp)$ \Comment{Halo exchange}
    \State $\alpha \gets \rho / \text{all\_reduce}(\bp^\top \bA\bp, \text{SUM})$
    \State $\bx \gets \bx + \alpha \bp$
    \State $\br \gets \br - \alpha \bA\bp$
    \State $\rho_{\text{new}} \gets \text{all\_reduce}(\br^\top \br, \text{SUM})$
    \State $\bp \gets \br + (\rho_{\text{new}}/\rho) \bp$
...
\label{alg:nonlinear}
\begin{algorithmic}[1]
\Require Residual $\bF(\bu, \btheta)$, initial guess $\bu_0$, parameters $\btheta$
\State \textbf{Forward:} Solve $\bF(\bu^*, \btheta) = \mathbf{0}$ via Newton
\For{$k = 0, 1, \ldots$}
    \State $\bJ \gets \partial \bF / \partial \bu$ \Comment{Jacobian via autograd}
    \State $\Delta \bu \gets \bJ^{-1} (-\bF)$ \Comment{Sparse linear solve}
    \State $\bu \gets \bu + \alpha \Delta \bu$ \Comment{Line search}
\EndFor
\State \textbf{Backward:} Given $\partial \mathcal{L}/\partial \bu^*$
\State Solve $\bJ^\top \blambda = \partial \mathcal{L}/\partial \bu^*$ \Comment{Adjoint equation}
\State $\partial \mathcal{L}/\partial \btheta \gets -\blambda^\top \partial \bF / \partial \btheta$ \Comment{VJP via autograd}
\State \Return $\partial \mathcal{L}/\partial \btheta$
\end{algorithmic}
\end{algorithm}
