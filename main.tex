...
Sparse linear systems $\bA\bx = \bb$ are fundamental to scientific computing. They arise in finite element analysis~\citep{hughes2012finite}, graph neural networks~\citep{kipf2017semi}, physics-informed machine learning~\citep{raissi2019physics}, and computational fluid dynamics~\citep{jasak2007openfoam}. With the rise of differentiable programming, there is growing demand for sparse solvers that integrate with automatic differentiation frameworks like PyTorch~\citep{paszke2019pytorch}.
...
\begin{itemize}
    \item \textbf{Learned preconditioners}: Neural networks for adaptive preconditioning.
    \item \textbf{Mixed precision}: FP16/BF16 with FP64 accumulation.
    \item \textbf{Sparse-sparse products}: Efficient SpGEMM with gradients.
    \item \textbf{Integration}: Combining with neural operators~\citep{li2020fourier} for hybrid solvers.
\end{itemize}
